...
{  
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE);	//使能定时器3时钟
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_AFIO,ENABLE); //使能GPIO外设
...
void TIM4_PWM_Init(u16 arr,u16 psc)
{
   ...
   //设置该引脚为复用输出功能,输出TIM3 CH2的PWM脉冲波形	GPIOB.7
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;				 //PB.6 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
	GPIO_Init(GPIOB, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB.6
	GPIO_ResetBits(GPIOB,GPIO_Pin_6);						 //PB.6 输出低
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;				 //PB.9 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
